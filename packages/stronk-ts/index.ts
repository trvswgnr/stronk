...
const EXTENDS = Symbol.for("stronk.extends");
...
/* eslint-disable custom-rules/no-typeof */
/**
    ...
    return Object.assign(
        (...args: any[]) => {
            if (typeof constructorMethod === "function") {
                ...
                return Object.assign(primitive, { [TYPE]: name, [EXTENDS]: typeof primitive });
            }
            return { [TYPE]: name };
        },
        {
            from: (x: any) => {
                return Object.assign(x, { [TYPE]: name, [EXTENDS]: typeof x });
            },
            [BASE]: name,
        },
    );
...
const Html = type.create("Html", HtmlConstructor);
type Html = type.infer<typeof Html>;
const Brand = type.create("Brand", Number);
type Brand = type.infer<typeof Brand>;

type HtmlConstructor = {
    readonly empty: Html;
    from(str: string): Html;
    from(strs: string[]): Html;
};
function HtmlConstructor(literals: TemplateStringsArray, ...placeholders: string[]): string;
function HtmlConstructor(
    literalsOrStr?: TemplateStringsArray | string,
    ...placeholders: string[]
): string {
    if (typeof literalsOrStr === "undefined") {
        return "";
    }
    if (typeof literalsOrStr === "string") {
        return literalsOrStr;
    }
    let str = "";
    for (let i = 0; i < literalsOrStr.length; i++) {
        str += literalsOrStr[i];
...
HtmlConstructor.empty = Html.from("");

const f = Html`<div></div>`;
const div: unknown = Html.from("<div></div>");

console.log(type.of(div));
console.log(type.of(23));
console.log(type.of(new String("23")));
console.log(type.of(null));
console.log(type.of(undefined));
console.log(type.of({}));
const takesHtml = (x: Html) => x;
if (type.is(div, Html)) {
    takesHtml(div);
    console.log("div is Html and string");
}

const MyNumber = type.create("MyNumber", Number);
type MyNumber = type.infer<typeof MyNumber>;
const myNum: unknown = MyNumber(23);
console.log(type.of(myNum));
console.log(type.is(myNum, MyNumber));
console.log(type.is(myNum, Number));
const takesNumber = (x: number) => x + 1;
const regNum = 69;
console.log(regNum.constructor.name);
